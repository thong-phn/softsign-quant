...
DATASETS=("mhealth" "realworld")
QUANT_METHODS=("linear" "gamma" "softsign")
...
for DATASET in "${DATASETS[@]}"; do
    ...
    for QUANT in "${QUANT_METHODS[@]}"; do
        ...
        python $MAIN_SCRIPT --dataset $TRAIN_DATASET --quantization $QUANT --run_name_prefix '1BIT'
    ...
    for QUANT in "${QUANT_METHODS[@]}"; do
        if [ "$QUANT" == "no" ]; then
            echo "Skipping Per-Channel configuration for Quantization=no"
            continue
        fi
        ...
        echo " -> Training & Evaluation (F32) [Script: $MAIN_SCRIPT]"
        python $MAIN_SCRIPT --dataset $TRAIN_DATASET --quantization $QUANT --per-channel-quant --run_name_prefix '1BIT'
