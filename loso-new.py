...
def main():
    ...
    parser.add_argument("--dataset", type=str, choices=['mhealth', 'recgym', 'realworld'], required=True)
    ...
    if dataset == 'recgym':
        ...
        all_subjects = list(range(1, 11))  # Subjects 1 to 10
        num_channels = 6
    elif dataset == 'realworld':
        root_path = project_root / "datasets" / "realworld"
        all_subjects = list(range(1, 16))  # Subjects 1 to 15
        num_channels = 9
    else:  # mhealth
    ...
    print(f"Total Subjects ({len(all_subjects)}): {all_subjects}")
    
    # Store metrics across folds
    fold_results = []

    # Determine which test folds to run
    test_folds = [fold_to_run] if fold_to_run is not None else all_subjects
    if fold_to_run is not None and fold_to_run not in all_subjects:
        print(f"[Warning] --fold {fold_to_run} is not in the list of subjects. Exit. ")
        return

    # Run LOSO (Outer loop: Test Subject, Inner loop: Val Subject)
    for test_subject in test_folds:
        ...
        for val_subject in remaining_subjects:
            ...
            print(f"\n{'='*50}")
            print(f"Fold (Test: {test_subject} | Val: {val_subject}) | Train Subjects: {len(train_subjects)}")
            print(f"{'='*50}")

            # Tracking init
            wandb_run = None
            if not args.no_wandb:
                wandb_name = f"[{args.run_name_prefix}]{dataset}-Test:{test_subject}-Val:{val_subject}-Q:{quantization}-PerC:{per_channel_quant}"
                if args.run_name:
                ...
                wandb_config = {
                    ...
                    "test_fold": test_subject,
                    "val_fold": val_subject
                }
            if dataset == 'uci':
            ...
            model_save_path = project_root / "models" / f"{prefix}_test_{test_subject}_val_{val_subject}.pth"
    ...
    with open(log_path, "w") as f:
        ...
        for res in fold_results:
            ...
            f.write(
                f"Fold Test {res['test_subject']} | Val {res['val_subject']}: Best Epoch = {best_epoch}, "
